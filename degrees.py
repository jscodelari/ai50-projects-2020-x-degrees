...
def shortest_path(source, target):
    ...
    Loop while frontier not empty or until solution
    ...
    # Prepare to leave as soon as we see the target while expanding.  Before we take a state from the frontier
    Eureka = False
    ...
    while True:
        ...
        for movie_id, person_id in neighbors_for_person(node.state):
                if not frontier.contains_state(person_id) and person_id not in explored:
                    ...
                    frontier.add(child)
                    # If the target is found, exit loop 
                    if (person_id == target):
                        Eureka = True
                        break
